#!/usr/bin/env python3
"""
Standalone Structural Analysis for 45S5/Sr Bioglass
Based on: Xiang & Du, Chem. Mater. 2011, 23, 2703-2717

Reads:  final_structure.xyz (output of Sr.py)
Writes: analysis_results.xlsx + plots/

Usage:
    python analyze.py Bioglass_x0_N11340_seed42/final_structure.xyz
    python analyze.py final_structure.xyz --output-dir my_analysis
"""
...
# Pair-specific peak search ranges for bond lengths
PEAK_RANGES = {
...
def run_analysis(xyz_path, output_dir=None):
    ...
    rdf_data = {}; bond_lengths = {}; cn_fixed = {}; cn_auto = {}
    for e1 in present:
        for e2 in present:
    ...
    export_excel(results, rdf_data, angles, modifier_cn_dist, output_dir, x_val)
...
def export_excel(results, rdf_data, angles, modifier_cn_dist, output_dir, x_val):
    path = output_dir / "analysis_results.xlsx"
    with pd.ExcelWriter(path, engine='openpyxl') as writer:
        pd.DataFrame([results['metadata'].items()],
                     columns=[k for k in results['metadata'].keys()]
                     ).T.to_excel(writer, sheet_name='Metadata', header=['Value'])
        ...
        for (e1, e2), cn in results['cn_fixed'].items():
            ...
            cn_rows.append({'Pair': f'{e1}-{e2}', 'Cutoff_A': cutoff,
                            'CN_fixed': cn, 'CN_auto': cn_auto_val})
        pd.DataFrame(cn_rows).to_excel(writer, sheet_name='Coordination_Numbers', index=False)

        mod_rows = [{'Modifier': m, 'CN': c, 'Paper_CN': PAPER_CN.get(m, np.nan)}
                    for m, c in results['modifier_cn'].items()]
        pd.DataFrame(mod_rows).to_excel(writer, sheet_name='Modifier_CN', index=False)
        ...
        pd.DataFrame(nc_rows).to_excel(writer, sheet_name='Network_Connectivity', index=False)
        ...
        pd.DataFrame(os_rows).to_excel(writer, sheet_name='Oxygen_Speciation', index=False)
        ...
        for nf, data in results['cn_around_nf'].items():
            rows = [{'Modifier': m, 'CN': c} for m, c in data.items()]
            pd.DataFrame(rows).to_excel(writer, sheet_name=f'CN_around_{nf}', index=False)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Standalone structural analysis for 45S5/Sr bioglass")
    parser.add_argument("input", help="Path to final_structure.xyz")
    parser.add_argument("--output-dir", type=str, default=None, help="Output directory")
    args = parser.parse_args()
    run_analysis(args.input, output_dir=args.output_dir)