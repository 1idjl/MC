...
"""
========================================================================
45S5/Mg Bioglass NVT Monte Carlo - Paper-based version
Based on: Moghanian et al., Mg-doped 45S5 bioglass manuscript

This is the Mg-adapted version of your Sr Monte Carlo code.

Paper-based inputs:
- Composition from Table 2
- Densities from Table 2
- Buckingham parameters from Table 1
- Partial charges:
  Si +2.4, Ca +1.2, Na +0.6, P +3.0, Mg +1.2, O -1.2

Usage:
python structure_generator_paper_mg.py --mg 5 --scale 4 --seed 42
python simulation_mg.py initial_Mg5.xyz --final-sweeps 150 --cores 12

For continued relaxation:
python simulation_mg.py final_structure.xyz --continue --final-sweeps 300
========================================================================
"""
...
# Paper-reported approximate reference values
PAPER_BOND_LENGTHS = {
    ("Si", "O"): 1.606,
    ("P", "O"): 1.50,
    ("Ca", "O"): 2.36,
...
class SimulationConfig:
    ...
    # MC schedule. Temperatures are chosen to mimic melt-quench/relaxation.
...
class Potential:
    def __init__(self):
        # ZBL repulsion for very short distances
        self.zbl_a0 = 0.46850
        ...
        # Buckingham parameters from manuscript Table 1
        ...
        # Short-range hard-core switching.
        # These are numerical safeguards for MC, not paper Buckingham terms.
        ...
        # Used only for the empirical Fnet index.
        # The manuscript does not explicitly tabulate SBS_Mg; Ca-like divalent value is used.
        self.sbs_x_o = {
            "Ca": 32.0,
            "Na": 20.0,
            "Mg": 32.0,
...
class MCSimulator:
    def __init__(self, system: GlassSystem, config: SimulationConfig):
        ...
        self.snapshot_interval = (
            ...
            if config.snapshot_interval is not None
            else max(N, self.final_steps // max(1, config.max_snapshots))
        )

        self.tune_freq = (
            config.adaptive_tuning_freq
            if config.adaptive_tuning_freq is not None
            else max(500, N // 10)
        )
        ...
        self.decomposition_freq = (
            config.decomposition_freq
            if config.decomposition_freq is not None
            else max(5000, N)
        )
    ...
    def run(self):
        logger.info("=" * 70)
        logger.info(f"STARTING NVT MONTE CARLO - Mg label x={self.config.x}")
...
class Analyzer:
    def __init__(self, system, config):
        ...
        # Analysis cutoffs.
        # Si-O, P-O, Na-O, Ca-O are consistent with common 45S5 analysis.
        # Mg-O cutoff is chosen around the first Mg-O shell, consistent with
        # paper Mg-O bond length ~1.98 A and CN ~4.1.
        self.FIXED_CUTOFFS = {
            ("Si", "O"): 2.25,
            ("P", "O"): 2.25,
            ("Na", "O"): 3.34,
        ...
        self.SBS_X_O = system.potential.sbs_x_o

        self.ALL_PAIRS = [
            p for p in self.FIXED_CUTOFFS
    ...
    def compute_rdf(self, coords, types, ei, ej, box, cut, rmax=6.0, nbins=400):
        ...
        if self.use_freud:
            try:
                ...
                if np.any(mask_cn):
                    ...
                    cn = trapezoid(
                        gr[mask_cn] * 4.0 * np.pi * r[mask_cn] ** 2 * rho,
                        r[mask_cn]
                    )
                else:
                    cn = 0.0
        ...
        if ni <= nj:
            ...
            small_idx = np.where(mj)[0]
            large_coords = coords[mi]
        ...
        for idx in small_idx:
            ...
            dx = xi - large_coords[:, 0]
            dy = yi - large_coords[:, 1]
            dz = zi - large_coords[:, 2]
        ...
        vol = 4.0 * np.pi * rc ** 2 * dr

        norm_factor = (
            box ** 3 / (ni * (ni - 1)) if ti == tj else box ** 3 / (ni * nj)
        )
        ...
        if np.any(mask_cn):
            rho = (nj - 1 if ti == tj else nj) / box ** 3
            cn = trapezoid(
                gr[mask_cn] * 4.0 * np.pi * rc[mask_cn] ** 2 * rho,
                rc[mask_cn]
            )
        else:
            cn = 0.0
    ...
    def _compute_angles(self, coords, types, box):
        ...
        for si in si_idx:
            ...
            for k in range(starts[si], starts[si + 1]):
                ...
                if types[j] == self.type_map["O"]:
                    dx = coords[si, 0] - coords[j, 0]
                    dy = coords[si, 1] - coords[j, 1]
                    dz = coords[si, 2] - coords[j, 2]
            ...
            for a in range(len(o_neighbors)):
                for b in range(a + 1, len(o_neighbors)):
                    ...
                    osi_angles.append(
                        np.degrees(np.arccos(np.clip(cos_ang, -1, 1)))
                    )
        ...
        for o in o_idx:
            for k in range(starts[o], starts[o + 1]):
                ...
                if types[j] == self.type_map["Si"]:
                    dx = coords[o, 0] - coords[j, 0]
                    dy = coords[o, 1] - coords[j, 1]
                    dz = coords[o, 2] - coords[j, 2]
        ...
        for o, sil in enumerate(o_si):
            if len(sil) == 2:
                v1 = coords[sil[0]] - coords[o]
                v2 = coords[sil[1]] - coords[o]
    ...
    def analyze_snapshot(self, snap):
        ...
        for pair in self.ALL_PAIRS:
            ...
            if auto_cut is not None and auto_cut > 0.5:
                ...
                if np.any(mask_cn):
                    ...
                    cn_auto_val = trapezoid(
                        4.0 * np.pi * r_cn ** 2 * rho * gr_cn,
                        r_cn
                    )
                else:
                    cn_auto_val = 0.0
        ...
        type_si = self.type_map["Si"]
        type_p = self.type_map["P"]
        type_o = self.type_map["O"]
        ...
        for o in o_idx:
            ...
            for k in range(starts[o], starts[o + 1]):
                ...
                dy -= bx * np.round(dy / bx)
                dz -= bx * np.round(dz / bx)

                r = sqrt(dx * dx + dy * dy + dz * dz)

                if ts[j] == type_si and r < si_o_cut:
                    nf_types.append(type_si)
                elif ts[j] == type_p and r < p_o_cut:
                    nf_types.append(type_p)

            o_nf.append(len(nf_types))
        ...
        for nf_t in o_nf_types:
            if len(nf_t) == 2:
                if nf_t[0] == type_si and nf_t[1] == type_si:
                    bo_types["Si-O-Si"] += 1
                elif nf_t[0] == type_si and nf_t[1] == type_p:
                    bo_types["Si-O-P"] += 1
                elif nf_t[0] == type_p and nf_t[1] == type_p:
                    bo_types["P-O-P"] += 1

        bridging_set = {
            o_idx[i] for i, c in enumerate(o_nf) if c >= 2
        }
        ...
        for si in si_idx:
            ...
            for k in range(starts[si], starts[si + 1]):
                ...
                if ts[j] == type_o:
                    ...
                    if r < si_o_cut and j in bridging_set:
                        bcnt += 1

            si_bo.append(min(bcnt, 4))

        qn_si = {
            n: int(np.sum(np.array(si_bo) == n))
            for n in range(5)
        }
            ...
            for p in p_idx:
                ...
                for k in range(starts[p], starts[p + 1]):
                    ...
                    if ts[j] == type_o:
                        ...
                        if r < p_o_cut and j in bridging_set:
                            bcnt += 1

                p_bo.append(min(bcnt, 4))

            qn_p = {
                n: int(np.sum(np.array(p_bo) == n))
                for n in range(5)
            }
        else:
        ...
        qn_c = {
            n: (qn_si[n] + qn_p[n]) / N_net * 100.0 if N_net > 0 else 0.0
            for n in range(5)
        }
        ...
        for A in ["Si", "P"]:
            for B, C in [("Mg", "Ca"), ("Ca", "Na"), ("Mg", "Na")]:
                ...
                if CN_AC > 0:
                    modifier_preference[key] = (CN_AB / CN_AC) * (NC / NB)
                else:
                    modifier_preference[key] = 0.0
        ...
        return {
            ...
            "osi_mean": osi_mean,
            "siosi_mean": siosi_mean,

            "p_si_links": p_si_links,
            "total_p_o_bonds": total_p_o_bonds,
        }
    ...
    def analyze_all(self, snapshots, n_cores=4):
        ...
        results = []
        for snap in tqdm(snapshots, desc="Analysis"):
            results.append(self.analyze_snapshot(snap))
        ...
        cn_fixed_dict = defaultdict(list)
        cn_auto_dict = defaultdict(list)
        auto_cutoff_dict = defaultdict(list)
        cn_extra_dict = defaultdict(lambda: defaultdict(list))
        bo_types_dict = defaultdict(list)

        for res in results:
            for pair, val in res["cn_fixed"].items():
                cn_fixed_dict[pair].append(val)

            for pair, val in res["cn_auto"].items():
                cn_auto_dict[pair].append(val)

            for pair, val in res["auto_cutoff"].items():
                auto_cutoff_dict[pair].append(val)

            if "cn_extra" in res:
                for pair, extra_vals in res["cn_extra"].items():
                    for rcut, val in extra_vals.items():
                        cn_extra_dict[pair][rcut].append(val)

            for k, v in res["bo_types"].items():
                bo_types_dict[k].append(v)
        ...
        agg["bo_types"] = {k: mean_std(v) for k, v in bo_types_dict.items()}

        for key in ["qn_si", "qn_p", "qn_combined"]:
            d = defaultdict(list)
            for res in results:
    ...
    def compute_cn_curves(self, excel_path, rmax=6.0, output_dir=None):
        ...
        for pair in self.ALL_PAIRS:
            ei, ej = pair
            n_atom2[f"{ei}-{ej}"] = (
                counts.get(ei, 0) if ei == ej else counts.get(ej, 0)
            )
        ...
        for sheet in rdf_sheets:
            ...
            integrand = 4.0 * np.pi * r_masked ** 2 * rho * gr_masked
            cn = np.zeros_like(r_masked)

            for i in range(1, len(r_masked)):
                cn[i] = cn[i - 1] + trapezoid(
                    integrand[i - 1:i + 1],
                    r_masked[i - 1:i + 1]
                )
        ...
        for j in range(len(cn_results), len(axes)):
    ...
    def export_excel(
        ...
        with pd.ExcelWriter(path, engine="openpyxl") as writer:
            meta_rows = [
                ["System", "45S5 Mg-doped bioactive glass (paper parameters)"],
                ["Mg_label", f"45-M{self.config.x}"],
                ["x_mol_percent_MgO", self.config.x],
                ...
                ["N_Mg", self.system.counts.get("Mg", 0)],
            ...
            if energy_data is not None and len(energy_data["total"]) > 0:
                meta_rows.append(
                    ["Final_Energy_eV_per_atom", f"{energy_data['total'][-1]:.6f}"]
                )

            if block_energy is not None:
                meta_rows.append(
                    ["Block_Avg_Energy_eV_per_atom", f"{block_energy['mean']:.6f}"]
                )
                meta_rows.append(
                    ["Block_Avg_Std_eV_per_atom", f"{block_energy['std']:.6f}"]
                )
            ...
            for pair in self.ALL_PAIRS:
                ...
                row = {
                    ...
                    "CN_fixed": mean_fixed,
                    "CN_fixed_se": se_fixed,
                    "Cutoff_fixed": cut_fixed,
                    "CN_auto": mean_auto,
                    "CN_auto_se": se_auto,
                    "Cutoff_auto": cut_auto,
                }
            ...
            qn_rows = [
                {
                    "Qn": f"Q{n}",
                    "Count": results["qn_si"][n][0],
                    "Count_se": results["qn_si"][n][1],
                    "Percentage": results["qn_si"][n][0] / N_SI * 100.0,
                }
                for n in range(5)
            ]
            ...
            if self.system.counts.get("P", 0) > 0:
                ...
                qn_rows = [
                    {
                        "Qn": f"Q{n}",
                        "Count": results["qn_p"][n][0],
                        "Count_se": results["qn_p"][n][1],
                        "Percentage": results["qn_p"][n][0] / N_P * 100.0,
                    }
                    for n in range(5)
                ]
            ...
            qn_rows = [
                {
                    "Qn": f"Q{n}",
                    "Percent": results["qn_combined"][n][0],
                    "Percent_se": results["qn_combined"][n][1],
                }
                for n in range(5)
            ]

            pd.DataFrame(qn_rows).to_excel(
                writer,
            ...
            pd.DataFrame({
                ...
                "Percentage": [
                    results["bo"][0] / N_O * 100.0,
                    results["to"][0] / N_O * 100.0,
                    results["nbo"][0] / N_O * 100.0,
                    results["fo"][0] / N_O * 100.0,
                ],
            }).to_excel(writer, sheet_name="O_speciation", index=False)

            if "bo_types" in results:
                rows = []
                for k in ["Si-O-Si", "Si-O-P", "P-O-P"]:
                    mean, se = results["bo_types"].get(k, (0.0, 0.0))
                    rows.append({
                        "Type": k,
                        "Count": mean,
                        "Count_se": se,
                        "Percentage_of_total_O": mean / N_O * 100.0,
                    })
            ...
            pd.DataFrame({
                "NC": [results["nc"][0]],
                "NC_se": [results["nc"][1]],
            }).to_excel(writer, sheet_name="NC", index=False)

            rxx_rows = [
                {
                    "Element": e,
                    "R_XX": results["r_xx"][e][0],
                    "R_XX_se": results["r_xx"][e][1],
                }
                for e in ["Ca", "Na", "Mg"]
                if e in results["r_xx"]
            ]
            ...
            if pref_rows:
                pd.DataFrame(pref_rows).to_excel(
                    writer,
                    sheet_name="Modifier_Preference",
                    index=False
                )

            pd.DataFrame({
                "Fnet_new": [results["fnet"][0]],
                "Fnet_new_se": [results["fnet"][1]],
                "SBS_Ca": [32.0],
                "SBS_Na": [20.0],
                "SBS_Mg": [32.0],
            ...
            if sensitivity:
                pd.DataFrame(sensitivity).to_excel(
                    writer,
            ...
            if final_coords is not None and final_types is not None and box is not None:
                for pair in self.ALL_PAIRS:
                    ...
                    df_rdf.to_excel(
                        writer,
                        sheet_name=f"RDF_{ei}-{ej}",
                        index=False
                    )

                osi_angles, siosi_angles = self._compute_angles(
                    final_coords,
                    final_types,
                    box
                )
                ...
                data = {"Angle_bin_center": centers}

                if osi_angles:
                    hist_osi, _ = np.histogram(osi_angles, bins=bins)
                    data["O-Si-O_frequency"] = hist_osi

                if siosi_angles:
                    hist_siosi, _ = np.histogram(siosi_angles, bins=bins)
                    data["Si-O-Si_frequency"] = hist_siosi

                if len(data) > 1:
                    pd.DataFrame(data).to_excel(
                        writer,
                        sheet_name="Angle_Distribution",
                        index=False
                    )

        self.compute_cn_curves(
            path,
            rmax=6.0,
            output_dir=self.config.output_dir / "plots"
        )
    ...
    def plot_results(self, results, outdir):
        ...
        for idx, pair in enumerate(self.ALL_PAIRS):
            ...
            axes[idx].set_xlabel("r (A)")
            axes[idx].set_ylabel("g(r)")
            axes[idx].set_title(f"{pair[0]}-{pair[1]}")
            axes[idx].legend(fontsize=8)
            axes[idx].grid(alpha=0.3)

        for extra in range(len(self.ALL_PAIRS), len(axes)):
            axes[extra].axis("off")

        plt.tight_layout()
        plt.savefig(outdir / "rdf.png", dpi=300, bbox_inches="tight")
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig(outdir / "qn.png", dpi=300, bbox_inches="tight")
        plt.close()
...
class SensitivityAnalyzer:
    def __init__(self, system, config):
        ...
        for (e1, e2), val in self.pot.r_hard_by_pair.items():
            t1 = self.system.type_map[e1]
            t2 = self.system.type_map[e2]
            self.R_HARD[t1, t2] = val
            self.R_HARD[t2, t1] = val
    ...
    def run(self):
        ...
        for cut in self.config.test_cutoffs:
            for alpha in self.config.test_alphas:
                ...
                logger.info(
                    f"  Cutoff={cut:.1f} A, alpha={alpha:.2f}: E={U_tot:.6f} eV/atom"
                )
...
def print_summary(results, x_val):
    ...
    for pair, bl in sorted(results.get("bond_lengths", {}).items()):
        if np.isnan(bl):
            continue

        paper = PAPER_BOND_LENGTHS.get(
            pair,
            PAPER_BOND_LENGTHS.get((pair[1], pair[0]), None)
        )

        ps = f"{paper:.3f}" if paper else "  -  "
        flag = "OK" if paper and abs(bl - paper) < 0.15 else ""

        print(f"  {pair[0]}-{pair[1]:2s}: {bl:.3f}  (paper: {ps}) {flag}")
    ...
    for mod, cn in results.get("modifier_cn", {}).items():
        ...
        flag = "OK" if paper and abs(cn - paper) < 0.8 else ""

        print(f"  {mod}-O: {cn:.2f}  (paper: {ps}) {flag}")

    print("\n--- Network Connectivity ---")
    nc_val, nc_se = results.get("nc", (0.0, 0.0))
    paper_nc = PAPER_NC.get(int(x_val), None)
    ps = f"{paper_nc:.3f}" if paper_nc is not None else "  -  "

    print(f"  NC: {nc_val:.3f} +/- {nc_se:.3f}  (paper: {ps})")

    print("\n--- Oxygen Speciation ---")
    o_spec = results.get("oxygen_speciation", {})
    tot = sum(o_spec.values())

    if tot > 0:
        for k, v in o_spec.items():
            print(f"  {k:4s}: {v:8.1f}  ({v / tot * 100:5.2f}%)")

    print("\n--- BO types ---")
    bo_types = results.get("bo_types", {})
    for k, v in bo_types.items():
        print(f"  {k:8s}: {v[0]:8.1f} +/- {v[1]:.2f}")

    print("\n--- Clustering R_XX ---")
    for mod, (rxx, se) in results.get("r_xx", {}).items():
        print(f"  {mod}: R={rxx:.3f} +/- {se:.3f}")

    print("\n--- Qn Distribution (%) ---")

    if "qn_si_percent" in results:
        print(
            "  Si:  " +
            "  ".join(
                f"{k}:{v[0]:.1f}"
                for k, v in results["qn_si_percent"].items()
            )
        )

    if "qn_p_percent" in results:
        print(
            "  P:   " +
            "  ".join(
                f"{k}:{v[0]:.1f}"
                for k, v in results["qn_p_percent"].items()
            )
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="45S5/Mg Bioglass NVT Monte Carlo - paper-based version"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="Number of CPU cores requested for analysis (serial analyzer is robust)."
    )
    parser.add_argument(
        "--final-sweeps",
        type=int,
    ...
    try:
        ...
        results = analyzer.analyze_all(snapshots, n_cores=args.cores)
        results["block_energy"] = block_energy

        # Bond lengths from final structure: use RDF peak positions
        bond_lengths = {}

        for pair in analyzer.ALL_PAIRS:
            cn, peak, r, gr = analyzer.compute_rdf(
                final_coords,
                final_types,
                pair[0],
                pair[1],
                box,
                analyzer.FIXED_CUTOFFS[pair],
                rmax=6.0,
                nbins=400
            )

            if peak > 0:
                bond_lengths[pair] = peak
        ...
        # Qn percentages
        N_SI = max(1, system.counts.get("Si", 0))
        N_P = max(1, system.counts.get("P", 0))

        results["qn_si_percent"] = {
            f"Q{n}": (
                results["qn_si"][n][0] / N_SI * 100.0,
                results["qn_si"][n][1] / N_SI * 100.0,
            )
            for n in range(5)
        }

        if system.counts.get("P", 0) > 0:
            results["qn_p_percent"] = {
                f"Q{n}": (
                    results["qn_p"][n][0] / N_P * 100.0,
                    results["qn_p"][n][1] / N_P * 100.0,
                )
                for n in range(5)
            }

        # Oxygen speciation dictionary for summary
        results["oxygen_speciation"] = {
            "FO": results["fo"][0],
            "NBO": results["nbo"][0],
            "BO": results["bo"][0],
            "TO": results["to"][0],
