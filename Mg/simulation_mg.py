#!/usr/bin/env python3
"""
45S5/Mg Bioglass NVT Monte Carlo - v10.1 (IDENTITY SWAP FIX APPLIED)
Based on: Moghanian et al., Mg-doped 45S5 bioglass manuscript

v10.1 CRITICAL FIX:
  - Replaced physical coordinate swap with CHEMICAL IDENTITY SWAP.
  - This correctly equilibrates cation distribution (R-factor) without 
    breaking the neighbor list or the glass network.

Usage:
  python simulation_mg_v10.py initial_Mg5.xyz --final-sweeps 50000
"""

import numpy as np
import os, sys, logging, re, warnings, codecs
...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler('simulation_v10.log', 'w', 'utf-8'), UTF8StreamHandler()]
)
...
class SimulationConfig:
    ...
    annealing_sweeps: List[Tuple[float, int]] = field(default_factory=lambda: [
        (3000, 6), (2500, 6), (2000, 10), (1500, 10), (1200, 12),
        (2000, 8), (1500, 10), (1000, 14), (2000, 8), (1500, 10),
        (1000, 14), (800, 12), (700, 12), (600, 10), (500, 8),
        (400, 6), (300, 6)
    ])

    p_osi_breaking_temp: float = 1500.0
    p_osi_breaking_sweeps: int = 10

    healing_temp: float = 2500.0
    healing_sweeps: int = 5
    low_t_sweeps: int = 8

    final_sweeps: int = 500 

    snapshot_interval: Optional[int] = None
    max_snapshots: int = 50  # Increased for better statistical averaging
...
class Potential:
    def __init__(self):
        ...
        self.r_hard_by_pair = {('O', 'O'): 1.4, ('Mg', 'O'): 1.0}
...
class MCSimulator:
    def __init__(self, system: GlassSystem, config: SimulationConfig):
        ...
        self.p_osi_breaking_steps = max(1, int(config.p_osi_breaking_sweeps * N))
        ...
        logger.info(
            ...
            f"P-O-Si breaking={self.p_osi_breaking_steps}, "
            f"healing={self.healing_steps}, low-T={self.low_t_steps}, "
            f"production={self.final_steps}"
        )
        logger.info(f"Identity Swap move frequency: every {config.swap_move_freq} steps")

    def _build_cation_lists(self):
        self.na_indices = np.where(self.system.type_indices == self.system.type_map['Na'])[0]
        self.ca_indices = np.where(self.system.type_indices == self.system.type_map['Ca'])[0]
        self.mg_indices = np.where(self.system.type_indices == self.system.type_map['Mg'])[0]
    ...
    def _swap_move(self, T: float) -> bool:
        """
        CHEMICAL IDENTITY SWAP:
        Swaps the element type and charge of two cations without moving their coordinates.
        This correctly equilibrates the cation distribution (crucial for R-factor) 
        while keeping the neighbor list perfectly valid.
        """
        available = []
        if self.na_indices.size > 0: available.append(('Na', self.na_indices))
        if self.ca_indices.size > 0: available.append(('Ca', self.ca_indices))
        if self.mg_indices.size > 0: available.append(('Mg', self.mg_indices))
        ...
        # 1. Calculate old energy
        e_i_old = local_energy(i, self.system.coords, self.system.charges,
                               self.system.type_indices, self.type_Z,
                               self.A_mat, self.F_mat, self.C_mat, self.R_HARD_MAT,
                               self.system.box, self.config.cutoff, self.config.wolf_alpha,
                               neigh, st,
        ...
        e_j_old = local_energy(j, self.system.coords, self.system.charges,
                               self.system.type_indices, self.type_Z,
                               self.A_mat, self.F_mat, self.C_mat, self.R_HARD_MAT,
                               self.system.box, self.config.cutoff, self.config.wolf_alpha,
                               neigh, st,
        ...
        # 3. Swap identities (Coordinates DO NOT change)
        ...
        e_i_new = local_energy(i, self.system.coords, self.system.charges,
                               self.system.type_indices, self.type_Z,
                               self.A_mat, self.F_mat, self.C_mat, self.R_HARD_MAT,
                               self.system.box, self.config.cutoff, self.config.wolf_alpha,
                               neigh, st,
        ...
        e_j_new = local_energy(j, self.system.coords, self.system.charges,
                               self.system.type_indices, self.type_Z,
                               self.A_mat, self.F_mat, self.C_mat, self.R_HARD_MAT,
                               self.system.box, self.config.cutoff, self.config.wolf_alpha,
                               neigh, st,
        ...
        if de <= 0.0 or self.rng.random() < exp(-de / (KB_EV * T)):
            ...
            # Note: No need to rebuild neighbor list because coordinates didn't move!
        else:
            # Revert swap
    ...
    def _run_stage(self, T, steps, desc, adaptive=False, mixing=False,
    ...
    def run(self):
        ...
        if self.config.continue_mode:
            ...
            for i, (T, steps) in enumerate(self.annealing_stages):
                self._run_stage(T, steps, f"Annealing {i+1}", adaptive=True,
                                enhance_oxygen=True, enable_swap=True)

            logger.info(f"P-O-Si Breaking Stage: T={self.config.p_osi_breaking_temp:.0f} K")
            self._run_stage(self.config.p_osi_breaking_temp, self.p_osi_breaking_steps,
                            "P-O-Si Breaking", adaptive=True, enhance_oxygen=True, enable_swap=True)

            for k in self.max_disp: self.max_disp[k] *= 0.7
            self._run_stage(self.config.healing_temp, self.healing_steps,
                            "Defect Healing", adaptive=True, enhance_oxygen=True, enable_swap=True)
            self._run_stage(1000.0, max(1, int(3 * self.system.N_ATOMS)),
                            "Cool after healing", adaptive=True, enable_swap=True)
        ...
        logger.info(f"Swap moves: attempts={self.swap_attempts}, accepted={self.swap_accepted}")

# ========== CLI ==========
app = typer.Typer(help="45S5/Mg Bioglass NVT MC v10.1 - Identity Swap Fix")

@app.command()
def run(
    input_file: Path = typer.Argument(..., help="Input XYZ file"),
    seed: Optional[int] = typer.Option(None, help="Random seed"),
    density: Optional[float] = typer.Option(None, help="Density override (g/cm3)"),
    final_sweeps: Optional[int] = typer.Option(None, help="Production MC sweeps"),
    cutoff: float = typer.Option(12.0, help="Cutoff (A)"),
    wolf_alpha: float = typer.Option(0.20, help="Wolf alpha"),
    continue_mode: bool = typer.Option(False, "--continue", help="Continue mode"),
):
    try:
        ...
        config = SimulationConfig(
            ...
            cutoff=cutoff, wolf_alpha=wolf_alpha, continue_mode=continue_mode
        )
        ...
        with open(xyz_path, 'w') as f:
            ...
            f.write(
                f"Final Moghanian et al., label=45-M{config.x}, "
                f"x={config.x}, N={system.N_ATOMS}, rho={system.effective_density:.4f} g/cm3, "
        ...
        logger.info(f"[OK] Simulation complete. Results in {config.output_dir}")
