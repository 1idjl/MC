...
Usage:
  ...
  python Sr.py Bioglass_x0_N11340_seed42/final_structure.xyz --continue --final-sweeps 300 --cores 12
"""
...
import pandas as pd
import os, sys, logging, pickle, time, re, warnings, codecs
...
class Potential:
    def __init__(self):
        ...
        self.buck_params = {
            ...
            # Repulsive terms to prevent Si-Si, Si-P, P-P bonds
    ...
    def _setup_box(self):
        if self.config.box_override is not None:
            ...
            if self.config.density is not None:
                ...
                if diff > 0.01:
                    logger.warning(f"Box density {self.effective_density:.4f} differs from "
                                   f"provided {self.config.density:.4f}. Using box from XYZ.")
...
class MCSimulator:
    ...
    def _check_geometry(self, idx):
        ...
        Checks: Si-Si, Si-P, P-P (min 3.0 A) and O-O (min 2.0 A).
        Called BEFORE energy calculation for efficiency.
...
def run(
    ...
    try:
        ...
        if len(prod_energy) >= 6:
            ...
            mean_e = float(np.mean(prod_energy)); std_e = 0.0
        block_energy = {'mean': mean_e, 'std': std_e}
