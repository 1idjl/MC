...
def parse_xyz_header(path: Path):
    ...
    if m:
        meta['density'] = float(m.group(1))

    m = re.search(r"box\s*=\s*([0-9]*\.?[0-9]+)", second)
    if m:
        meta['box'] = float(m.group(1))
...
class SimulationConfig:
    ...
    def __post_init__(self):
        if self.output_dir is None:
            self.output_dir = Path(f"Bioglass_x{self.x}_seed{self.seed}")
        else:
...
class GlassSystem:
    ...
    def _load_xyz(self):
        ...
        if self.N_ATOMS != n_header:
            logger.warning(
                f"XYZ header says {n_header} atoms, but {self.N_ATOMS} atom lines were read."
            )

        if self.N_ATOMS % BASE_N_ATOMS != 0:
            logger.warning(
                f"Total atoms {self.N_ATOMS} is not an integer multiple of {BASE_N_ATOMS}."
            )
    ...
    def _setup_box(self):
        if self.config.box_override is not None:
            ...
            if self.config.density is not None:
                diff = abs(self.effective_density - self.config.density)
                if diff > 0.01:
                    logger.warning(
                        f"Box-derived density {self.effective_density:.4f} differs from "
            ...
            raise FileError(
                "Neither box nor density could be determined. "
                "Provide density or use an XYZ file with box/rho metadata."
            )
...
# =========================
class NeighborList:
    def __init__(self, system: GlassSystem, cutoff: float, skin: float = SKIN):
        self.system = system
        self.cutoff = cutoff
        self.skin = skin
        self.sc = cutoff + skin
    ...
    def _needs_rebuild(self):
        ...
        max_dr = np.sqrt(np.sum(dr**2, axis=1)).max()

        return max_dr > self.skin / 3.0
...
class MCSimulator:
    def __init__(self, system: GlassSystem, config: SimulationConfig):
        ...
        self.annealing_stages = [
            (T, max(1, int(sweeps * N)))
            for T, sweeps in config.annealing_sweeps
        ]
        ...
        logger.info(
            f"MC schedule: mixing={self.mixing_steps} steps, "
            f"annealing={sum(s for _, s in self.annealing_stages)} steps, "
            f"healing={self.healing_steps} steps, "
            f"low-T={self.low_t_steps} steps, production={self.final_steps} steps"
        )

        logger.info(
            f"Logging: log_freq={self.log_freq}, "
            f"decomposition_freq={self.decomposition_freq}, "
            f"snapshot_interval={self.snapshot_interval}, "
            f"tune_freq={self.tune_freq}"
        )
    ...
    def run(self):
        ...
        logger.info(
            ...
            f"neighbor rebuilds: {self.rebuilds}"
        )
...
class Analyzer:
    ...
    def _find_first_minimum(self, r, gr, pair, rmin=None, rmax=4.0):
        ...
        idx_min = np.argmin(gr_smooth)
        return r_masked[idx_min]
    ...
    def compute_rdf(self, coords, types, ei, ej, box, cut, rmax=6.0, nbins=400):
        ...
        if ti == tj:
            norm_factor = box**3 / (ni * (ni - 1))
        else:
            norm_factor = box**3 / (ni * nj)
    ...
    def _compute_angles(self, coords, types, box):
        ...
        for si in si_idx:
            ...
            for a in range(len(o_neighbors)):
                for b in range(a + 1, len(o_neighbors)):
                    ...
                    osi_angles.append(np.degrees(np.arccos(np.clip(cos_ang, -1.0, 1.0))))
        ...
        for o, sil in enumerate(o_si):
            if len(sil) == 2:
                ...
                cos_ang = np.dot(v1, v2) / (np.linalg.norm(v1) * np.linalg.norm(v2))
                siosi_angles.append(np.degrees(np.arccos(np.clip(cos_ang, -1.0, 1.0))))
    ...
    def analyze_snapshot(self, snap):
        ...
        for pair in self.ALL_PAIRS:
            ...
            cn, peak, r, gr = self.compute_rdf(
                cs, ts, ei, ej, bx, cut_fixed, rmax=6.0, nbins=400
            )
            ...
            if auto_cut is not None and auto_cut > 0.5:
                ...
                if np.any(mask_cn):
                    ...
                    integrand = 4.0 * np.pi * r_cn**2 * rho * gr_cn
                    cn_auto_val = trapezoid(integrand, r_cn)
                else:
                    cn_auto_val = 0.0
            ...
            if pair == ('Si', 'O'):
                ...
                for rcut in self.si_o_cut_extra:
                    ...
                    if np.any(mask_cn):
                        ...
                        integrand = 4.0 * np.pi * r_cn**2 * rho * gr_cn
                        cn_val = trapezoid(integrand, r_cn)
                    else:
                        cn_val = 0.0
        ...
        for o in o_idx:
            ...
            for k in range(starts[o], starts[o + 1]):
                ...
                if ts[j] == self.type_map['Si'] and r < si_o_cut:
                    nsi += 1
                elif ts[j] == self.type_map['P'] and r < p_o_cut:
                    np_ += 1
    ...
    def analyze_all(self, snapshots, n_cores=4):
        ...
        if len(snapshots) <= 1 or n_cores <= 1:
            ...
            with Pool(processes=min(n_cores, len(snapshots))) as pool:
                results = list(tqdm(
                    pool.imap(self.analyze_snapshot, snapshots),
                    total=len(snapshots)
                ))
    ...
    def compute_cn_curves(self, excel_path, rmax=6.0, output_dir=None):
        ...
        for pair in self.ALL_PAIRS:
            ...
            if ei == ej:
                n_atom2[f"{ei}-{ej}"] = counts.get(ei, 0)
            else:
                n_atom2[f"{ei}-{ej}"] = counts.get(ej, 0)
    ...
    def export_excel(self, results, path, energy_data=None, final_coords=None, final_types=None, box=None):
...
def run(
    ...
    density: Optional[float] = typer.Option(
        None,
        help="Optional density override in g/cm^3."
    ),
    cores: int = typer.Option(
        4,
        help="Number of CPU cores for analysis."
    ),
    final_sweeps: Optional[int] = typer.Option(
        None,
        help="Override production MC sweeps."
    ),
    cutoff: float = typer.Option(
        10.0,
        help="Effective short-range + Wolf Coulomb cutoff in Angstrom."
    ),
    ...
    try:
        ...
        if len(prod_energy) >= 6:
            ...
            mean_e = float(np.mean(prod_energy))
            std_e = 0.0
