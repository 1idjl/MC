#!/usr/bin/env python3
"""
45S5/Sr Bioglass NVT Monte Carlo - v9.0 (Pure Simulation)
Based on: Xiang & Du, Chem. Mater. 2011, 23, 2703-2717

v9.0 Changes:
- REMOVED hard geometric constraints (smart initial structure handles it)
- REMOVED artificial Si-Si / Si-P / P-P repulsive potentials
- REMOVED Analyzer & SensitivityAnalyzer (see analyze.py)
- RESTORED original Table 2 potentials only
- RESTORED O-O r_hard = 1.4 (paper value)

Usage:
    python Sr.py initial_Sr0.xyz --final-sweeps 150 --cores 8
    python Sr.py final_structure.xyz --continue --final-sweeps 300
"""
...
# ========== POTENTIAL (Table 2 only, no artificial terms) ==========
class Potential:
    def __init__(self):
        self.zbl_a0 = 0.46850
        ...
        # Only O-O hard radius (original paper value)
...
class MCSimulator:
    ...
    def _mc_move(self, T: float) -> bool:
        ...
        # NO geometric constraint - physics handles it naturally
...
# ========== CLI ==========
app = typer.Typer(help="45S5/Sr Bioglass NVT Monte Carlo - v9.0 Pure Simulation")
...
def run(
    ...
    cutoff: float = typer.Option(10.0, help="Cutoff (A)"),
    wolf_alpha: float = typer.Option(0.25, help="Wolf alpha"),
    ...
    try:
        ...
        # Save metadata
        meta_path = config.output_dir / "simulation_meta.txt"
        with open(meta_path, 'w') as f:
            f.write(f"x_mol_percent_SrO = {config.x}\n")
            f.write(f"N_atoms = {system.N_ATOMS}\n")
            f.write(f"box_A = {system.box:.6f}\n")
            f.write(f"density_g_cm3 = {system.effective_density:.6f}\n")
            f.write(f"seed = {config.seed}\n")
            f.write(f"cutoff_A = {config.cutoff}\n")
            f.write(f"wolf_alpha = {config.wolf_alpha}\n")
            f.write(f"final_sweeps = {config.final_sweeps}\n")
            f.write(f"continue_mode = {config.continue_mode}\n")
            f.write(f"total_attempts = {sim.attempts}\n")
            f.write(f"acceptance_rate = {sim.accepted / max(1, sim.attempts) * 100:.2f}%\n")
            f.write(f"final_energy_eV_per_atom = {sim.current_energy / system.N_ATOMS:.6f}\n")
            for e, c in system.counts.items():
                f.write(f"N_{e} = {c}\n")
